...
class OCRMapper:
    ...
    FIELD_MAPPINGS = {
        ...
        'customer_address': ['customer_address', 'customer_addr', 'billing_address', 'bill_to_address'],
    ...
    @staticmethod
    def _find_field_value(extracted_fields: Dict[str, Any], field_aliases: list[str]) -> Optional[Any]:
    ...
    def map_ocr_to_invoice(self, ocr_input: OCRInput) -> InvoiceCreate:
        ...
        logger.info("Starting OCR to Invoice mapping", extra={
            "extracted_fields_count": len(extracted),
            "confidence_score": ocr_input.confidence_score
        })
        
        # Map invoice number (required)
        invoice_number = self._find_field_value(extracted, self.FIELD_MAPPINGS['invoice_number'])
        if not invoice_number:
            errors['invoice_number'] = "Invoice number is required but not found in OCR data"
        
        # Map invoice date (required)
        invoice_date_raw = self._find_field_value(extracted, self.FIELD_MAPPINGS['invoice_date'])
        invoice_date = self._parse_date(invoice_date_raw)
        if not invoice_date:
            errors['invoice_date'] = f"Invoice date is required but not found or invalid: {invoice_date_raw}"
        
        # Map due date (optional)
        due_date_raw = self._find_field_value(extracted, self.FIELD_MAPPINGS['due_date'])
        due_date = self._parse_date(due_date_raw)
        
        # Map vendor information (required)
        vendor_name = self._find_field_value(extracted, self.FIELD_MAPPINGS['vendor_name'])
        if not vendor_name:
            errors['vendor_name'] = "Vendor name is required but not found in OCR data"
        
        vendor_address = self._find_field_value(extracted, self.FIELD_MAPPINGS['vendor_address'])
        vendor_tax_id = self._find_field_value(extracted, self.FIELD_MAPPINGS['vendor_tax_id'])
        
        # Map customer information (optional)
        customer_name = self._find_field_value(extracted, self.FIELD_MAPPINGS['customer_name'])
        customer_address = self._find_field_value(extracted, self.FIELD_MAPPINGS['customer_address'])
        
        # Map financial information
        subtotal_raw = self._find_field_value(extracted, self.FIELD_MAPPINGS['subtotal'])
        subtotal = self._parse_decimal(subtotal_raw)
        
        tax_amount_raw = self._find_field_value(extracted, self.FIELD_MAPPINGS['tax_amount'])
        tax_amount = self._parse_decimal(tax_amount_raw)
        
        total_amount_raw = self._find_field_value(extracted, self.FIELD_MAPPINGS['total_amount'])
        total_amount = self._parse_decimal(total_amount_raw)
        if not total_amount or total_amount <= 0:
            errors['total_amount'] = f"Total amount is required and must be positive: {total_amount_raw}"
        
        currency = self._find_field_value(extracted, self.FIELD_MAPPINGS['currency']) or "USD"
        ...
        invoice_data = InvoiceCreate(
            ...
            confidence_score=Decimal(str(ocr_input.confidence_score)) if ocr_input.confidence_score else None
        )
        
        logger.info("Successfully mapped OCR data to invoice", extra={
