...
class InvoiceValidator:
    ...
    def validate_invoice(self, invoice: InvoiceCreate) -> Dict[str, Any]:
        ...
        logger.info("Starting invoice validation", extra={
        ...
        if not self._validate_currency(invoice.currency):
            errors.append(f"Currency '{invoice.currency}' is not supported. Valid currencies: {', '.join(self.VALID_CURRENCIES)}")
        
        # Validate vendor information
        if not self._validate_vendor_name(invoice.vendor_name):
            errors.append("Vendor name is too short or contains only special characters")
    ...
    def _validate_dates(
        ...
        if invoice_date < now - timedelta(days=self.MAX_INVOICE_AGE_DAYS):
            warnings.append(f"Invoice date is more than {self.MAX_INVOICE_AGE_DAYS} days old")
